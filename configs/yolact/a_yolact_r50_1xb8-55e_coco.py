...
model = dict(
    ...
    train_cfg=dict(
        ...
        sampler=dict(type='PseudoSampler'),  # YOLACT 使用 PseudoSampler
        allowed_border=-1,
        pos_weight=-1,
        neg_pos_ratio=3,
        debug=False),
...
# 数据集设置
train_pipeline = [
    dict(type='LoadImageFromFile'),  # 修复 backend_args 引用
...
custom_hooks = [
    # dict(
    #     type='TensorboardLoggerHook',  # 启用 Tensorboard 记录
    #     interval=50,  # 每50步记录一次
...
train_dataloader = dict(
    ...
    dataset=dict(
        ...
        data_prefix=dict(img='images/train2017/')  # 训练图像路径
    )
)
...
val_dataloader = dict(
    ...
    dataset=dict(
        ...
        data_prefix=dict(img='images/val2017/')  # 验证图像路径
    )
)
...
val_evaluator = dict(
    ...
    classwise=True  # 按类输出评估结果
)

test_evaluator = val_evaluator  # 使用相同的评估器进行测试
